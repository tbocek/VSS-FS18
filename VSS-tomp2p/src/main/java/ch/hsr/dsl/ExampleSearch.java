...
public final class ExampleSearch {
    private static final String TERM = "Institut für Software";
    ...
    private static void exampleKeywordSearch(final PeerDHT[] peers) throws IOException, ClassNotFoundException {
        ...
        Number160 termKey = findReference(peers[peer20], "für");
        if(termKey == null) {
        ...
        futureGet.awaitUninterruptibly();
        System.out.println("searched (keyword) for [für], found " + futureGet.data().object());
    }
    ...
    private static void exampleXORSearch(final PeerDHT[] peers) throws IOException, ClassNotFoundException {
        ...
        // search for a keyword
        Number160 key1Keyword = Number160.createHash("Institut");
        Number160 key2Keyword = Number160.createHash("Software");
        Number160 termKey = findReference(peers[20], key1Keyword.xor(key2Keyword));

        if(termKey != null) {
            FutureGet futureGet = peers[10].get(termKey).start();
            futureGet.awaitUninterruptibly();
            System.out.println("searched (xor) for [Institut and Software], found " + futureGet.data().object());
        } else {
            System.out.println("searched (xor) for [Institut and Software], nothing found");
        }
    }
    ...
    private static void exampleBloomfilerSearch(final PeerDHT[] peers) throws IOException, ClassNotFoundException {
        ...
        Number160 termKey = findReference(peers[20], Number160.createHash("Institut"), Number160.createHash("Software"));

        if(termKey != null) {
            FutureGet futureGet = peers[10].get(termKey).start();
            futureGet.awaitUninterruptibly();
            System.out.println("searched (bf) for [Institut and Software], found " + futureGet.data().object());
        } else {
            System.out.println("searched (bf) for [Institut and Software], nothing found");
        }
    }
